...
LABELLED_URLS: List[Tuple[str, float, str]] = [
    ...
    # --- Added to exercise fixes from the KNOWN WEAKNESSES list --------------
    # #11: an opinion subdomain should NOT inherit its parent's full reputation.
    ("https://blogs.nytimes.com/dealbook/2024/example-opinion-piece", 0.40, "opinion blog subdomain of a mainstream outlet, not straight reporting"),
    # #5: an unrecognized .edu domain with a personal-directory path.
    ("https://www.cs.cmu.edu/~jdoe/index.html", 0.35, "personal faculty/student homepage on .edu, not institutionally vetted"),
    # #4: a real retracted paper — requires network access to Crossref (free,
    # keyless). If your machine can't complete outbound HTTPS from Python
    # (some corporate/antivirus TLS-inspection setups break stdlib `ssl`
    # trust even though curl and browsers work fine), this signal silently
    # won't fire and the row will show as an outlier — that's Layer 1.5
    # degrading exactly as designed, not a scoring bug. Pass --no-network to
    # exclude this layer and confirm. See retraction_check() and README.
    ("https://doi.org/10.1016/S0140-6736(20)31180-6", 0.05, "retracted (Surgisphere/hydroxychloroquine study, Lancet 2020)"),
]


def evaluate(use_llm: bool, use_network: bool = True) -> Dict[str, float]:
    ...
    for url, expected, rationale in LABELLED_URLS:
        result = score_url(url, use_llm=use_llm if use_llm else False, use_network=use_network)
        got = result["score"]
    ...
    print(f"  Network layer      : {'on (Crossref retraction check)' if use_network else 'off'}\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--no-network", action="store_true", help="disable the Crossref retraction check (Layer 1.5) for a fully offline run")
    ...
    evaluate(use_llm=args.llm, use_network=not args.no_network)